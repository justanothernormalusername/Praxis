...
def plan(learning_details: str) -> dict:
    ...
    with open("backend/out/spec.json", "w", encoding="utf-8") as file:
...
def build(spec: dict) -> None:
    ...
    if not spec:
        with open("backend/test_spec.json", "r") as file:
            spec = json.loads(file.read())
    
    build_output = requests.post("http://127.0.0.1:8000/build", json=spec).json()

    with open("backend/out.json", "w", encoding="utf-8") as file:
...
def generate() -> None:
    with open("backend/out.json", "r", encoding="utf-8") as out:
        build_output = json.loads(out.read())
        files = build_output["files"]
        for file_raw in files:
            print(f"backend/out/{file_raw["title"].replace(" ", "_")}")
            with open(f"backend/out/{file_raw["title"].replace(" ", "_")}", "w", encoding="utf-8") as file:
                file.write(file_raw["data"])
    shutil.make_archive("backend/out/pset", "zip", "backend/out/")
