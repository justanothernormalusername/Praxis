...
def generate() -> None:
    with open("out.json", "r", encoding="utf-8") as out:
        build_output = json.loads(out.read())
        files = build_output["files"]
        for file_raw in files:
            print(f"out/{file_raw["title"].replace(" ", "_")}")
            with open(f"out/{file_raw["title"].replace(" ", "_")}", "w", encoding="utf-8") as file:
                file.write(file_raw["data"])
    shutil.make_archive("out/pset", "zip", "out/")
